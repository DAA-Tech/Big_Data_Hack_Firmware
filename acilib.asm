...
;acilib.c,35 :: 		void acil_encode_cmd_set_test_mode(uint8_t *buffer, aci_cmd_params_test_t *p_aci_cmd_params_test)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_test start address is: 4 (R1)
;acilib.c,37 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = 2;
MOVS	R2, #2
STRB	R2, [R0, #0]
;acilib.c,38 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_TEST;
ADDS	R3, R0, #1
MOVS	R2, #1
STRB	R2, [R3, #0]
;acilib.c,39 :: 		*(buffer + OFFSET_ACI_CMD_T_TEST + OFFSET_ACI_CMD_PARAMS_TEST_T_TEST_MODE_CHANGE) = p_aci_cmd_params_test->test_mode_change;
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
LDRB	R2, [R1, #0]
; p_aci_cmd_params_test end address is: 4 (R1)
...
;acilib.c,42 :: 		void acil_encode_cmd_sleep(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,44 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = 1;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,45 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_SLEEP;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #4
STRB	R1, [R2, #0]
...
;acilib.c,48 :: 		void acil_encode_cmd_get_device_version(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,50 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = 1;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,51 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_GET_DEVICE_VERSION;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #9
STRB	R1, [R2, #0]
...
;acilib.c,54 :: 		void acil_encode_cmd_set_local_data(uint8_t *buffer, aci_cmd_params_set_local_data_t *p_aci_cmd_params_set_local_data, uint8_t data_size)
...
; p_aci_cmd_params_set_local_data start address is: 4 (R1)
; data_size start address is: 8 (R2)
;acilib.c,56 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_SET_LOCAL_DATA_BASE_LEN + data_size;
ADDS	R3, R2, #2
STRB	R3, [R0, #0]
;acilib.c,57 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_SET_LOCAL_DATA;
ADDS	R4, R0, #1
MOVS	R3, #13
STRB	R3, [R4, #0]
;acilib.c,58 :: 		*(buffer + OFFSET_ACI_CMD_T_SET_LOCAL_DATA + OFFSET_ACI_CMD_PARAMS_SEND_DATA_T_TX_DATA + OFFSET_ACI_TX_DATA_T_PIPE_NUMBER) = p_aci_cmd_params_set_local_data->tx_data.pipe_number;
ADDS	R4, R0, #2
LDRB	R3, [R1, #0]
STRB	R3, [R4, #0]
;acilib.c,59 :: 		memcpy(buffer + OFFSET_ACI_CMD_T_SET_LOCAL_DATA + OFFSET_ACI_CMD_PARAMS_SEND_DATA_T_TX_DATA + OFFSET_ACI_TX_DATA_T_ACI_DATA,  &(p_aci_cmd_params_set_local_data->tx_data.aci_data[0]), data_size);
ADDS	R4, R1, #1
; p_aci_cmd_params_set_local_data end address is: 4 (R1)
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
...
;acilib.c,62 :: 		void acil_encode_cmd_connect(uint8_t *buffer, aci_cmd_params_connect_t *p_aci_cmd_params_connect)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_connect start address is: 4 (R1)
;acilib.c,64 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_CONNECT_LEN;
MOVS	R2, #5
STRB	R2, [R0, #0]
;acilib.c,65 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_CONNECT;
ADDS	R3, R0, #1
MOVS	R2, #15
STRB	R2, [R3, #0]
;acilib.c,66 :: 		*(buffer + OFFSET_ACI_CMD_T_CONNECT + OFFSET_ACI_CMD_PARAMS_CONNECT_T_TIMEOUT_MSB) = (uint8_t)(p_aci_cmd_params_connect->timeout >> 8);
ADDS	R2, R0, #2
ADDS	R3, R2, #1
LDRH	R2, [R1, #0]
LSRS	R2, R2, #8
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,67 :: 		*(buffer + OFFSET_ACI_CMD_T_CONNECT + OFFSET_ACI_CMD_PARAMS_CONNECT_T_TIMEOUT_LSB) = (uint8_t)(p_aci_cmd_params_connect->timeout);
ADDS	R3, R0, #2
LDRH	R2, [R1, #0]
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,68 :: 		*(buffer + OFFSET_ACI_CMD_T_CONNECT + OFFSET_ACI_CMD_PARAMS_CONNECT_T_ADV_INTERVAL_MSB) = (uint8_t)(p_aci_cmd_params_connect->adv_interval >> 8);
ADDS	R2, R0, #2
ADDS	R3, R2, #3
ADDS	R2, R1, #2
LDRH	R2, [R2, #0]
...
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,69 :: 		*(buffer + OFFSET_ACI_CMD_T_CONNECT + OFFSET_ACI_CMD_PARAMS_CONNECT_T_ADV_INTERVAL_LSB) = (uint8_t)(p_aci_cmd_params_connect->adv_interval);
ADDS	R2, R0, #2
; buffer end address is: 0 (R0)
ADDS	R3, R2, #2
ADDS	R2, R1, #2
...
;acilib.c,89 :: 		void acil_encode_baseband_reset(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,91 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_BASEBAND_RESET_LEN;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,92 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_RADIO_RESET;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #14
STRB	R1, [R2, #0]
...
;acilib.c,95 :: 		void acil_encode_direct_connect(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,97 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_DIRECT_CONNECT_LEN;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,98 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_CONNECT_DIRECT;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #30
STRB	R1, [R2, #0]
...
;acilib.c,101 :: 		void acil_encode_cmd_wakeup(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,103 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_WAKEUP_LEN;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,104 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_WAKEUP;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #5
STRB	R1, [R2, #0]
...
;acilib.c,107 :: 		void acil_encode_cmd_set_radio_tx_power(uint8_t *buffer, aci_cmd_params_set_tx_power_t *p_aci_cmd_params_set_tx_power)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_set_tx_power start address is: 4 (R1)
;acilib.c,109 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_SET_RADIO_TX_POWER_LEN;
MOVS	R2, #2
STRB	R2, [R0, #0]
;acilib.c,110 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_SET_TX_POWER;
ADDS	R3, R0, #1
MOVS	R2, #18
STRB	R2, [R3, #0]
;acilib.c,111 :: 		*(buffer + OFFSET_ACI_CMD_T_SET_TX_POWER + OFFSET_ACI_CMD_PARAMS_SET_TX_POWER_T_DEVICE_POWER) = (uint8_t)p_aci_cmd_params_set_tx_power->device_power;
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
LDRB	R2, [R1, #0]
; p_aci_cmd_params_set_tx_power end address is: 4 (R1)
...
;acilib.c,114 :: 		void acil_encode_cmd_get_address(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,116 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_GET_DEVICE_ADDR_LEN;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,117 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_GET_DEVICE_ADDRESS;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #10
STRB	R1, [R2, #0]
...
;acilib.c,128 :: 		void acil_encode_cmd_request_data(uint8_t *buffer, aci_cmd_params_request_data_t *p_aci_cmd_params_request_data)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_request_data start address is: 4 (R1)
;acilib.c,130 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_DATA_REQUEST_LEN;
MOVS	R2, #2
STRB	R2, [R0, #0]
;acilib.c,131 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_REQUEST_DATA;
ADDS	R3, R0, #1
MOVS	R2, #23
STRB	R2, [R3, #0]
;acilib.c,132 :: 		*(buffer + OFFSET_ACI_CMD_T_REQUEST_DATA + OFFSET_ACI_CMD_PARAMS_REQUEST_DATA_T_PIPE_NUMBER) = p_aci_cmd_params_request_data->pipe_number;
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
LDRB	R2, [R1, #0]
; p_aci_cmd_params_request_data end address is: 4 (R1)
...
;acilib.c,135 :: 		void acil_encode_cmd_open_remote_pipe(uint8_t *buffer, aci_cmd_params_open_remote_pipe_t *p_aci_cmd_params_open_remote_pipe)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_open_remote_pipe start address is: 4 (R1)
;acilib.c,137 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_OPEN_REMOTE_PIPE_LEN;
MOVS	R2, #2
STRB	R2, [R0, #0]
;acilib.c,138 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_OPEN_REMOTE_PIPE;
ADDS	R3, R0, #1
MOVS	R2, #20
STRB	R2, [R3, #0]
;acilib.c,139 :: 		*(buffer + OFFSET_ACI_CMD_T_OPEN_REMOTE_PIPE + OFFSET_ACI_CMD_PARAMS_OPEN_REMOTE_PIPE_T_PIPE_NUMBER) = p_aci_cmd_params_open_remote_pipe->pipe_number;
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
LDRB	R2, [R1, #0]
; p_aci_cmd_params_open_remote_pipe end address is: 4 (R1)
...
;acilib.c,142 :: 		void acil_encode_cmd_close_remote_pipe(uint8_t *buffer, aci_cmd_params_close_remote_pipe_t *p_aci_cmd_params_close_remote_pipe)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_close_remote_pipe start address is: 4 (R1)
;acilib.c,144 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_CLOSE_REMOTE_PIPE_LEN;
MOVS	R2, #2
STRB	R2, [R0, #0]
;acilib.c,145 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_CLOSE_REMOTE_PIPE;
ADDS	R3, R0, #1
MOVS	R2, #31
STRB	R2, [R3, #0]
;acilib.c,146 :: 		*(buffer + OFFSET_ACI_CMD_T_CLOSE_REMOTE_PIPE + OFFSET_ACI_CMD_PARAMS_CLOSE_REMOTE_PIPE_T_PIPE_NUMBER) = p_aci_cmd_params_close_remote_pipe->pipe_number;
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
LDRB	R2, [R1, #0]
; p_aci_cmd_params_close_remote_pipe end address is: 4 (R1)
...
;acilib.c,149 :: 		void acil_encode_cmd_echo_msg(uint8_t *buffer, aci_cmd_params_echo_t *p_cmd_params_echo, uint8_t msg_size)
...
; p_cmd_params_echo start address is: 4 (R1)
; msg_size start address is: 8 (R2)
;acilib.c,151 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_ECHO_MSG_CMD_BASE_LEN + msg_size;
ADDS	R3, R2, #1
STRB	R3, [R0, #0]
;acilib.c,152 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_ECHO;
ADDS	R4, R0, #1
MOVS	R3, #2
STRB	R3, [R4, #0]
;acilib.c,153 :: 		memcpy((buffer + OFFSET_ACI_CMD_T_ECHO + OFFSET_ACI_CMD_PARAMS_ECHO_T_ECHO_DATA), &(p_cmd_params_echo->echo_data[0]), msg_size);
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
; msg_size end address is: 8 (R2)
; p_cmd_params_echo end address is: 4 (R1)
...
;acilib.c,156 :: 		void acil_encode_cmd_battery_level(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,158 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = 1;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,159 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_GET_BATTERY_LEVEL;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #11
STRB	R1, [R2, #0]
...
;acilib.c,162 :: 		void acil_encode_cmd_temparature(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,164 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = 1;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,165 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_GET_TEMPERATURE;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #12
STRB	R1, [R2, #0]
...
;acilib.c,168 :: 		void acil_encode_cmd_read_dynamic_data(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,170 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = 1;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,171 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_READ_DYNAMIC_DATA;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #7
STRB	R1, [R2, #0]
...
;acilib.c,174 :: 		void acil_encode_cmd_write_dynamic_data(uint8_t *buffer, uint8_t seq_no, uint8_t* dynamic_data, uint8_t dynamic_data_size)
...
; dynamic_data start address is: 8 (R2)
; dynamic_data_size start address is: 12 (R3)
;acilib.c,176 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_WRITE_DYNAMIC_DATA_BASE_LEN + dynamic_data_size;
ADDS	R4, R3, #2
STRB	R4, [R0, #0]
;acilib.c,177 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_WRITE_DYNAMIC_DATA;
ADDS	R5, R0, #1
MOVS	R4, #8
STRB	R4, [R5, #0]
;acilib.c,178 :: 		*(buffer + OFFSET_ACI_CMD_T_WRITE_DYNAMIC_DATA + OFFSET_ACI_CMD_PARAMS_WRITE_DYNAMIC_DATA_T_SEQ_NO) = seq_no;
ADDS	R4, R0, #2
STRB	R1, [R4, #0]
; seq_no end address is: 4 (R1)
;acilib.c,179 :: 		memcpy((buffer + OFFSET_ACI_CMD_T_WRITE_DYNAMIC_DATA + OFFSET_ACI_CMD_PARAMS_WRITE_DYNAMIC_DATA_T_DYNAMIC_DATA), dynamic_data, dynamic_data_size);
ADDS	R4, R0, #2
; buffer end address is: 0 (R0)
ADDS	R4, R4, #1
MOV	R1, R2
...
;acilib.c,182 :: 		void acil_encode_cmd_change_timing_req(uint8_t *buffer, aci_cmd_params_change_timing_t *p_aci_cmd_params_change_timing)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_change_timing start address is: 4 (R1)
;acilib.c,184 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_CHANGE_TIMING_LEN;
MOVS	R2, #9
STRB	R2, [R0, #0]
;acilib.c,185 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_CHANGE_TIMING;
ADDS	R3, R0, #1
MOVS	R2, #19
STRB	R2, [R3, #0]
;acilib.c,186 :: 		*(buffer + OFFSET_ACI_CMD_T_CHANGE_TIMING + OFFSET_ACI_CMD_PARAMS_CHANGE_TIMING_T_CONN_PARAMS + OFFSET_ACI_LL_CONN_PARAMS_T_MIN_CONN_INTERVAL_MSB) = (uint8_t)(p_aci_cmd_params_change_timing->conn_params.min_conn_interval >> 8);
ADDS	R2, R0, #2
ADDS	R3, R2, #1
LDRH	R2, [R1, #0]
LSRS	R2, R2, #8
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,187 :: 		*(buffer + OFFSET_ACI_CMD_T_CHANGE_TIMING + OFFSET_ACI_CMD_PARAMS_CHANGE_TIMING_T_CONN_PARAMS + OFFSET_ACI_LL_CONN_PARAMS_T_MIN_CONN_INTERVAL_LSB) = (uint8_t)(p_aci_cmd_params_change_timing->conn_params.min_conn_interval);
ADDS	R3, R0, #2
LDRH	R2, [R1, #0]
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,188 :: 		*(buffer + OFFSET_ACI_CMD_T_CHANGE_TIMING + OFFSET_ACI_CMD_PARAMS_CHANGE_TIMING_T_CONN_PARAMS + OFFSET_ACI_LL_CONN_PARAMS_T_MAX_CONN_INTERVAL_MSB) = (uint8_t)(p_aci_cmd_params_change_timing->conn_params.max_conn_interval >> 8);
ADDS	R2, R0, #2
ADDS	R3, R2, #3
ADDS	R2, R1, #2
LDRH	R2, [R2, #0]
...
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,189 :: 		*(buffer + OFFSET_ACI_CMD_T_CHANGE_TIMING + OFFSET_ACI_CMD_PARAMS_CHANGE_TIMING_T_CONN_PARAMS + OFFSET_ACI_LL_CONN_PARAMS_T_MAX_CONN_INTERVAL_LSB) = (uint8_t)(p_aci_cmd_params_change_timing->conn_params.max_conn_interval);
ADDS	R2, R0, #2
ADDS	R3, R2, #2
ADDS	R2, R1, #2
LDRH	R2, [R2, #0]
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,190 :: 		*(buffer + OFFSET_ACI_CMD_T_CHANGE_TIMING + OFFSET_ACI_CMD_PARAMS_CHANGE_TIMING_T_CONN_PARAMS + OFFSET_ACI_LL_CONN_PARAMS_T_SLAVE_LATENCY_MSB    ) = (uint8_t)(p_aci_cmd_params_change_timing->conn_params.slave_latency >> 8);
ADDS	R2, R0, #2
ADDS	R3, R2, #5
ADDS	R2, R1, #4
LDRH	R2, [R2, #0]
...
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,191 :: 		*(buffer + OFFSET_ACI_CMD_T_CHANGE_TIMING + OFFSET_ACI_CMD_PARAMS_CHANGE_TIMING_T_CONN_PARAMS + OFFSET_ACI_LL_CONN_PARAMS_T_SLAVE_LATENCY_LSB    ) = (uint8_t)(p_aci_cmd_params_change_timing->conn_params.slave_latency);
ADDS	R2, R0, #2
ADDS	R3, R2, #4
ADDS	R2, R1, #4
LDRH	R2, [R2, #0]
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,192 :: 		*(buffer + OFFSET_ACI_CMD_T_CHANGE_TIMING + OFFSET_ACI_CMD_PARAMS_CHANGE_TIMING_T_CONN_PARAMS + OFFSET_ACI_LL_CONN_PARAMS_T_TIMEOUT_MULT_MSB     ) = (uint8_t)(p_aci_cmd_params_change_timing->conn_params.timeout_mult >> 8);
ADDS	R2, R0, #2
ADDS	R3, R2, #7
ADDS	R2, R1, #6
LDRH	R2, [R2, #0]
...
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,193 :: 		*(buffer + OFFSET_ACI_CMD_T_CHANGE_TIMING + OFFSET_ACI_CMD_PARAMS_CHANGE_TIMING_T_CONN_PARAMS + OFFSET_ACI_LL_CONN_PARAMS_T_TIMEOUT_MULT_LSB     ) = (uint8_t)(p_aci_cmd_params_change_timing->conn_params.timeout_mult);
ADDS	R2, R0, #2
; buffer end address is: 0 (R0)
ADDS	R3, R2, #6
ADDS	R2, R1, #6
...
;acilib.c,196 :: 		void acil_encode_cmd_set_app_latency(uint8_t *buffer, aci_cmd_params_set_app_latency_t *p_aci_cmd_params_set_app_latency)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_set_app_latency start address is: 4 (R1)
;acilib.c,198 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_SET_APP_LATENCY_LEN;
MOVS	R2, #4
STRB	R2, [R0, #0]
;acilib.c,199 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_SET_APP_LATENCY;
ADDS	R3, R0, #1
MOVS	R2, #25
STRB	R2, [R3, #0]
;acilib.c,200 :: 		*(buffer + OFFSET_ACI_CMD_T_SET_APP_LATENCY + OFFSET_ACI_CMD_PARAMS_SET_APP_LATENCY_T_MODE) = (uint8_t)( p_aci_cmd_params_set_app_latency->mode);
ADDS	R3, R0, #2
LDRB	R2, [R1, #0]
STRB	R2, [R3, #0]
;acilib.c,201 :: 		*(buffer + OFFSET_ACI_CMD_T_SET_APP_LATENCY + OFFSET_ACI_CMD_PARAMS_SET_APP_LATENCY_T_LATENCY_MSB) = (uint8_t)( p_aci_cmd_params_set_app_latency->latency>>8);
ADDS	R2, R0, #2
ADDS	R3, R2, #1
ADDS	R2, R1, #2
LDRH	R2, [R2, #0]
...
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,202 :: 		*(buffer + OFFSET_ACI_CMD_T_SET_APP_LATENCY + OFFSET_ACI_CMD_PARAMS_SET_APP_LATENCY_T_LATENCY_LSB) = (uint8_t)( p_aci_cmd_params_set_app_latency->latency);
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
ADDS	R2, R1, #2
; p_aci_cmd_params_set_app_latency end address is: 4 (R1)
...
;acilib.c,205 :: 		void acil_encode_cmd_change_timing_req_GAP_PPCP(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,207 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_CHANGE_TIMING_LEN_GAP_PPCP;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,208 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_CHANGE_TIMING;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #19
STRB	R1, [R2, #0]
...
;acilib.c,212 :: 		void acil_encode_cmd_setup(uint8_t *buffer, aci_cmd_params_setup_t *p_aci_cmd_params_setup, uint8_t setup_data_size)
...
; p_aci_cmd_params_setup start address is: 4 (R1)
; setup_data_size start address is: 8 (R2)
;acilib.c,214 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = setup_data_size + MSG_SETUP_CMD_BASE_LEN;
ADDS	R3, R2, #1
STRB	R3, [R0, #0]
;acilib.c,215 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_SETUP;
ADDS	R4, R0, #1
MOVS	R3, #6
STRB	R3, [R4, #0]
;acilib.c,216 :: 		memcpy((buffer + OFFSET_ACI_CMD_T_SETUP), &(p_aci_cmd_params_setup->setup_data[0]), setup_data_size);
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
; setup_data_size end address is: 8 (R2)
; p_aci_cmd_params_setup end address is: 4 (R1)
...
;acilib.c,219 :: 		void acil_encode_cmd_dtm_cmd(uint8_t *buffer, aci_cmd_params_dtm_cmd_t *p_aci_cmd_params_dtm_cmd)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_dtm_cmd start address is: 4 (R1)
;acilib.c,221 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_DTM_CMD;
MOVS	R2, #3
STRB	R2, [R0, #0]
;acilib.c,222 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_DTM_CMD;
ADDS	R3, R0, #1
MOVS	R2, #3
STRB	R2, [R3, #0]
;acilib.c,223 :: 		*(buffer + OFFSET_ACI_CMD_T_DTM_CMD) = p_aci_cmd_params_dtm_cmd->cmd_msb;
ADDS	R3, R0, #2
LDRB	R2, [R1, #0]
STRB	R2, [R3, #0]
;acilib.c,224 :: 		*(buffer + OFFSET_ACI_CMD_T_DTM_CMD + 1) = p_aci_cmd_params_dtm_cmd->cmd_lsb;
ADDS	R2, R0, #2
; buffer end address is: 0 (R0)
ADDS	R3, R2, #1
ADDS	R2, R1, #1
...
;acilib.c,227 :: 		void acil_encode_cmd_send_data_ack(uint8_t *buffer, const uint8_t pipe_number )
...
; buffer start address is: 0 (R0)
; pipe_number start address is: 4 (R1)
;acilib.c,229 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_ACK_LEN;
MOVS	R2, #2
STRB	R2, [R0, #0]
;acilib.c,230 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_SEND_DATA_ACK;
ADDS	R3, R0, #1
MOVS	R2, #22
STRB	R2, [R3, #0]
;acilib.c,231 :: 		*(buffer + OFFSET_ACI_CMD_T_SEND_DATA_ACK + OFFSET_ACI_CMD_PARAMS_SEND_DATA_ACK_T_PIPE_NUMBER) = pipe_number;
ADDS	R2, R0, #2
; buffer end address is: 0 (R0)
STRB	R1, [R2, #0]
; pipe_number end address is: 4 (R1)
...
;acilib.c,234 :: 		void acil_encode_cmd_send_data_nack(uint8_t *buffer, const uint8_t pipe_number, const uint8_t err_code )
...
; pipe_number start address is: 4 (R1)
; err_code start address is: 8 (R2)
;acilib.c,236 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_NACK_LEN;
MOVS	R3, #3
STRB	R3, [R0, #0]
;acilib.c,237 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_SEND_DATA_NACK;
ADDS	R4, R0, #1
MOVS	R3, #24
STRB	R3, [R4, #0]
;acilib.c,238 :: 		*(buffer + OFFSET_ACI_CMD_T_SEND_DATA_NACK + OFFSET_ACI_CMD_PARAMS_SEND_DATA_NACK_T_PIPE_NUMBER) = pipe_number;
ADDS	R3, R0, #2
STRB	R1, [R3, #0]
; pipe_number end address is: 4 (R1)
;acilib.c,239 :: 		*(buffer + OFFSET_ACI_CMD_T_SEND_DATA_NACK + OFFSET_ACI_CMD_PARAMS_SEND_DATA_NACK_T_ERROR_CODE) = err_code;
ADDS	R3, R0, #2
; buffer end address is: 0 (R0)
ADDS	R3, R3, #1
STRB	R2, [R3, #0]
...
;acilib.c,242 :: 		void acil_encode_cmd_bond_security_request(uint8_t *buffer)
; buffer start address is: 0 (R0)
; buffer end address is: 0 (R0)
; buffer start address is: 0 (R0)
;acilib.c,244 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = 1;
MOVS	R1, #1
STRB	R1, [R0, #0]
;acilib.c,245 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_BOND_SECURITY_REQUEST;
ADDS	R2, R0, #1
; buffer end address is: 0 (R0)
MOVS	R1, #29
STRB	R1, [R2, #0]
...
;acilib.c,248 :: 		void acil_encode_cmd_broadcast(uint8_t *buffer, aci_cmd_params_broadcast_t * p_aci_cmd_params_broadcast)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_broadcast start address is: 4 (R1)
;acilib.c,250 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_BROADCAST_LEN;
MOVS	R2, #5
STRB	R2, [R0, #0]
;acilib.c,251 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_BROADCAST;
ADDS	R3, R0, #1
MOVS	R2, #28
STRB	R2, [R3, #0]
;acilib.c,252 :: 		*(buffer + OFFSET_ACI_CMD_T_BROADCAST + OFFSET_ACI_CMD_PARAMS_BROADCAST_T_TIMEOUT_LSB) = (p_aci_cmd_params_broadcast->timeout & 0xff);
ADDS	R3, R0, #2
LDRH	R2, [R1, #0]
AND	R2, R2, #255
STRB	R2, [R3, #0]
;acilib.c,253 :: 		*(buffer + OFFSET_ACI_CMD_T_BROADCAST + OFFSET_ACI_CMD_PARAMS_BROADCAST_T_TIMEOUT_MSB) = (uint8_t)(p_aci_cmd_params_broadcast->timeout >> 8);
ADDS	R2, R0, #2
ADDS	R3, R2, #1
LDRH	R2, [R1, #0]
LSRS	R2, R2, #8
UXTB	R2, R2
STRB	R2, [R3, #0]
;acilib.c,254 :: 		*(buffer + OFFSET_ACI_CMD_T_BROADCAST + OFFSET_ACI_CMD_PARAMS_BROADCAST_T_ADV_INTERVAL_LSB) = (p_aci_cmd_params_broadcast->adv_interval & 0xff);
ADDS	R2, R0, #2
ADDS	R3, R2, #2
ADDS	R2, R1, #2
LDRH	R2, [R2, #0]
AND	R2, R2, #255
STRB	R2, [R3, #0]
;acilib.c,255 :: 		*(buffer + OFFSET_ACI_CMD_T_BROADCAST + OFFSET_ACI_CMD_PARAMS_BROADCAST_T_ADV_INTERVAL_MSB) = (uint8_t)(p_aci_cmd_params_broadcast->adv_interval >> 8);
ADDS	R2, R0, #2
; buffer end address is: 0 (R0)
ADDS	R3, R2, #3
ADDS	R2, R1, #2
...
;acilib.c,258 :: 		void acil_encode_cmd_open_adv_pipes(uint8_t *buffer, aci_cmd_params_open_adv_pipe_t * p_aci_cmd_params_open_adv_pipe)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_open_adv_pipe start address is: 4 (R1)
;acilib.c,260 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = MSG_OPEN_ADV_PIPES_LEN;
MOVS	R2, #9
STRB	R2, [R0, #0]
;acilib.c,261 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_OPEN_ADV_PIPE;
ADDS	R3, R0, #1
MOVS	R2, #27
STRB	R2, [R3, #0]
;acilib.c,262 :: 		memcpy(buffer + OFFSET_ACI_CMD_T_OPEN_ADV_PIPE + OFFSET_ACI_CMD_PARAMS_OPEN_ADV_PIPE_T_PIPES, p_aci_cmd_params_open_adv_pipe->pipes, 8);
ADDS	R2, R0, #2
; buffer end address is: 0 (R0)
MOV	R0, R2
; p_aci_cmd_params_open_adv_pipe end address is: 4 (R1)
...
;acilib.c,266 :: 		void acil_encode_cmd_set_key(uint8_t *buffer, aci_cmd_params_set_key_t *p_aci_cmd_params_set_key)
...
; buffer start address is: 0 (R0)
; p_aci_cmd_params_set_key start address is: 4 (R1)
;acilib.c,275 :: 		switch (p_aci_cmd_params_set_key->key_type)
MOV	R3, R1
IT	AL
BAL	L_acil_encode_cmd_set_key0
;acilib.c,277 :: 		case ACI_KEY_TYPE_INVALID:
L_acil_encode_cmd_set_key2:
;acilib.c,278 :: 		len = MSG_SET_KEY_REJECT_LEN;
...
;acilib.c,280 :: 		case ACI_KEY_TYPE_PASSKEY:
L_acil_encode_cmd_set_key3:
;acilib.c,281 :: 		len = MSG_SET_KEY_PASSKEY_LEN;
; len start address is: 20 (R5)
MOVS	R5, #8
;acilib.c,282 :: 		break;
; len end address is: 20 (R5)
...
;acilib.c,287 :: 		*(buffer + OFFSET_ACI_CMD_T_LEN) = len;
; len start address is: 20 (R5)
STRB	R5, [R0, #0]
;acilib.c,288 :: 		*(buffer + OFFSET_ACI_CMD_T_CMD_OPCODE) = ACI_CMD_SET_KEY;
ADDS	R3, R0, #1
MOVS	R2, #26
STRB	R2, [R3, #0]
;acilib.c,289 :: 		*(buffer + OFFSET_ACI_CMD_T_SET_KEY + OFFSET_ACI_CMD_PARAMS_SET_KEY_T_KEY_TYPE) = p_aci_cmd_params_set_key->key_type;
ADDS	R3, R0, #2
LDRB	R2, [R1, #0]
STRB	R2, [R3, #0]
;acilib.c,290 :: 		memcpy((buffer + OFFSET_ACI_CMD_T_SET_KEY + OFFSET_ACI_CMD_PARAMS_SET_KEY_T_PASSKEY), (uint8_t * )&(p_aci_cmd_params_set_key->key), len-2);//Reducing 2 for the opcode byte and type
SUBS	R4, R5, #2
; len end address is: 20 (R5)
ADDS	R3, R1, #1
; p_aci_cmd_params_set_key end address is: 4 (R1)
...
_acil_encode_cmd:
;acilib.c,293 :: 		char acil_encode_cmd(uint8_t *buffer, aci_cmd_t *p_aci_cmd)
...
;acilib.c,295 :: 		char ret_val = 0;
; ret_val start address is: 32 (R8)
MOVW	R8, #0
;acilib.c,297 :: 		switch(p_aci_cmd->cmd_opcode)
ADDS	R2, R7, #1
STR	R2, [SP, #4]
IT	AL
BAL	L_acil_encode_cmd5
;acilib.c,299 :: 		case ACI_CMD_TEST:
L_acil_encode_cmd7:
;acilib.c,300 :: 		acil_encode_cmd_set_test_mode(buffer, &(p_aci_cmd->params.test));
ADDS	R2, R7, #2
...
;acilib.c,302 :: 		case ACI_CMD_SLEEP:
L_acil_encode_cmd8:
;acilib.c,303 :: 		acil_encode_cmd_sleep(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,305 :: 		case ACI_CMD_GET_DEVICE_VERSION:
L_acil_encode_cmd9:
;acilib.c,306 :: 		acil_encode_cmd_get_device_version(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,308 :: 		case ACI_CMD_WAKEUP:
L_acil_encode_cmd10:
;acilib.c,309 :: 		acil_encode_cmd_wakeup(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,311 :: 		case ACI_CMD_ECHO:
L_acil_encode_cmd11:
;acilib.c,312 :: 		acil_encode_cmd_echo_msg(buffer, &(p_aci_cmd->params.echo), (p_aci_cmd->len - MSG_ECHO_MSG_CMD_BASE_LEN));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
LDRB	R2, [R7, #0]
SUBS	R3, R2, #1
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,314 :: 		case ACI_CMD_GET_BATTERY_LEVEL:
L_acil_encode_cmd12:
;acilib.c,315 :: 		acil_encode_cmd_battery_level(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,317 :: 		case ACI_CMD_GET_TEMPERATURE:
L_acil_encode_cmd13:
;acilib.c,318 :: 		acil_encode_cmd_temparature(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,320 :: 		case ACI_CMD_GET_DEVICE_ADDRESS:
L_acil_encode_cmd14:
;acilib.c,321 :: 		acil_encode_cmd_get_address(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,323 :: 		case ACI_CMD_SET_TX_POWER:
L_acil_encode_cmd15:
;acilib.c,324 :: 		acil_encode_cmd_set_radio_tx_power(buffer, &(p_aci_cmd->params.set_tx_power));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,326 :: 		case ACI_CMD_CONNECT:
L_acil_encode_cmd16:
;acilib.c,327 :: 		acil_encode_cmd_connect(buffer, &(p_aci_cmd->params.connect));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,329 :: 		case ACI_CMD_BOND:
L_acil_encode_cmd17:
;acilib.c,330 :: 		acil_encode_cmd_bond(buffer, &(p_aci_cmd->params.bond));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,332 :: 		case ACI_CMD_DISCONNECT:
L_acil_encode_cmd18:
;acilib.c,333 :: 		acil_encode_cmd_disconnect(buffer, &(p_aci_cmd->params.disconnect));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,335 :: 		case ACI_CMD_RADIO_RESET:
L_acil_encode_cmd19:
;acilib.c,336 :: 		acil_encode_baseband_reset(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,338 :: 		case ACI_CMD_CHANGE_TIMING:
L_acil_encode_cmd20:
;acilib.c,339 :: 		acil_encode_cmd_change_timing_req(buffer, &(p_aci_cmd->params.change_timing));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,341 :: 		case ACI_CMD_SETUP:
L_acil_encode_cmd21:
;acilib.c,342 :: 		acil_encode_cmd_setup(buffer, &(p_aci_cmd->params.setup), (p_aci_cmd->len - MSG_SETUP_CMD_BASE_LEN));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
LDRB	R2, [R7, #0]
SUBS	R3, R2, #1
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,344 :: 		case ACI_CMD_DTM_CMD:
L_acil_encode_cmd22:
;acilib.c,345 :: 		acil_encode_cmd_dtm_cmd(buffer, &(p_aci_cmd->params.dtm_cmd));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,347 :: 		case ACI_CMD_READ_DYNAMIC_DATA:
L_acil_encode_cmd23:
;acilib.c,348 :: 		acil_encode_cmd_read_dynamic_data(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,350 :: 		case ACI_CMD_WRITE_DYNAMIC_DATA:
L_acil_encode_cmd24:
;acilib.c,351 :: 		acil_encode_cmd_write_dynamic_data(buffer, p_aci_cmd->params.write_dynamic_data.seq_no, &(p_aci_cmd->params.write_dynamic_data.dynamic_data[0]), (p_aci_cmd->len - MSG_WRITE_DYNAMIC_DATA_BASE_LEN));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
LDRB	R2, [R7, #0]
SUBS	R4, R2, #2
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,353 :: 		case ACI_CMD_OPEN_REMOTE_PIPE:
L_acil_encode_cmd25:
;acilib.c,354 :: 		acil_encode_cmd_open_remote_pipe(buffer, &(p_aci_cmd->params.open_remote_pipe));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,356 :: 		case ACI_CMD_SEND_DATA:
L_acil_encode_cmd26:
;acilib.c,357 :: 		acil_encode_cmd_send_data(buffer, &(p_aci_cmd->params.send_data), (p_aci_cmd->len - MSG_SEND_DATA_BASE_LEN));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
LDRB	R2, [R7, #0]
SUBS	R3, R2, #2
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,359 :: 		case ACI_CMD_SEND_DATA_ACK:
L_acil_encode_cmd27:
;acilib.c,360 :: 		acil_encode_cmd_send_data_ack(buffer, p_aci_cmd->params.send_data_ack.pipe_number );
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,362 :: 		case ACI_CMD_REQUEST_DATA:
L_acil_encode_cmd28:
;acilib.c,363 :: 		acil_encode_cmd_request_data(buffer, &(p_aci_cmd->params.request_data));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
ADDS	R2, R7, #2
; p_aci_cmd end address is: 28 (R7)
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,365 :: 		case ACI_CMD_SET_LOCAL_DATA:
L_acil_encode_cmd29:
;acilib.c,366 :: 		acil_encode_cmd_set_local_data(buffer, (aci_cmd_params_set_local_data_t *)(&(p_aci_cmd->params.send_data)), (p_aci_cmd->len - MSG_SET_LOCAL_DATA_BASE_LEN));
; p_aci_cmd start address is: 28 (R7)
; buffer start address is: 24 (R6)
LDRB	R2, [R7, #0]
SUBS	R3, R2, #2
...
IT	AL
BAL	L_acil_encode_cmd6
;acilib.c,368 :: 		case ACI_CMD_BOND_SECURITY_REQUEST:
L_acil_encode_cmd30:
;acilib.c,369 :: 		acil_encode_cmd_bond_security_request(buffer);
; buffer start address is: 24 (R6)
MOV	R0, R6
; buffer end address is: 24 (R6)
...
SXTH	R2, R2
MOV	R1, R6
MOVW	R0, #lo_addr(_openPipes+0)
MOVT	R0, #hi_addr(_openPipes+0)
BL	_memcpy+0
...
MOVS	R2, #8
SXTH	R2, R2
MOVW	R0, #lo_addr(_openClosed+0)
MOVT	R0, #hi_addr(_openClosed+0)
BL	_memcpy+0
;acilib.c,474 :: 		}
...
;acilib.c,556 :: 		uint8_t acil_decode_evt_echo(uint8_t *buffer_in, aci_evt_params_echo_t *aci_evt_params_echo)
...
; buffer_in start address is: 0 (R0)
; aci_evt_params_echo start address is: 4 (R1)
;acilib.c,558 :: 		uint8_t size = *(buffer_in + OFFSET_ACI_EVT_T_LEN) - 1;
LDRB	R2, [R0, #0]
SUBS	R3, R2, #1
; size start address is: 24 (R6)
UXTB	R6, R3
;acilib.c,559 :: 		memcpy(&aci_evt_params_echo->echo_data[0], (buffer_in + OFFSET_ACI_EVT_T_EVT_OPCODE + 1), size);
ADDS	R2, R0, #1
; buffer_in end address is: 0 (R0)
ADDS	R2, R2, #1
MOV	R0, R1
