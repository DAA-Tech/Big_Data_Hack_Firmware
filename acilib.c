...
void acil_decode_evt_command_response(uint8_t *buffer_in, aci_evt_params_cmd_rsp_t *p_evt_params_cmd_rsp)
{
  ...
  UART_Write(13);
  UART_Write(10);
  IntToHex(p_evt_params_cmd_rsp->cmd_opcode, tmpStr);
  UART_Write_Text("Opcode: 0x");
  UART_Write_Text(tmpStr);
  
  UART_Write(13);
  UART_Write(10);
  if(p_evt_params_cmd_rsp->cmd_status == 0) {
    UART_Write_Text("Status:   OK");
  }
  else {
    IntToHex(p_evt_params_cmd_rsp->cmd_status, tmpStr);
    UART_Write_Text("Status:   0x");
    UART_Write_Text(tmpStr);
  }
  */
...
char acil_decode_evt(uint8_t *buffer_in, aci_evt_t *p_aci_evt)
{
  ...
  //UART_Debug(p_aci_evt->evt_opcode);
